...
module.exports.getAll = (event, context, callback) => {
  ...
  connectToDatabase().then(() => {
    Kortoon.find()
      ...
      .catch(err => callback(null, responseError(err.message)));
  });
};
...
module.exports.deleteAll = (event, context, callback) => {
  ...
  connectToDatabase().then(() => {
    Kortoon.remove({}, function(err, result) {
      if (err) {
        console.log(result);
        callback(null, responseError(err.message));
      } else {
        console.log(result);
        callback(null, responseSuccess(result));
      }
    });
  });
};
